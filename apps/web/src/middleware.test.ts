...
describe('auth middleware runtime contract', () => {
  ...
  it('uses the persisted landing cookie for authenticated auth-route redirects', () => {
    const token = createJwt({ mustChangePassword: false, role: 'superadmin' });
  ...
  it('redirects assistants away from admin-only direct access using their authorized snapshot', () => {
    const token = createJwt({ mustChangePassword: false, role: BaseRole.RECEPCIONISTA });
    const authorizedRoutes = getAuthorizedProtectedPaths(DEFAULT_ROLE_PERMISSIONS[BaseRole.RECEPCIONISTA]);
  ...
  it('allows admins to open audit routes when the snapshot contains the protected page', () => {
    const token = createJwt({ mustChangePassword: false, role: BaseRole.SUPERADMIN });
    const authorizedRoutes = getAuthorizedProtectedPaths(DEFAULT_ROLE_PERMISSIONS[BaseRole.SUPERADMIN]);
    const request = new NextRequest('http://localhost:3000/audit', {
      headers: {
